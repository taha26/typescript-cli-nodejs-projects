...
(async () => {
    ...
    while (status) {
        const { options }: { options: "Add item" | "Item List" | "Delete item" | "Delete all item" | "Exit" } = await Inquirer.prompt([
            {
                name: "options",
        ...
        if (options === "Add item") {
            const { item }: { item: string } = await Inquirer.prompt([
                {
                    name: "item",
        ...
        if (options === "Item List") {
            ...
            if (!todoList.length) console.log(chalk.red(`No item added`));
        ...
        if (options === "Delete item") {
            const { deleteItem }: { deleteItem: string } = await Inquirer.prompt([
                {
                    name: "deleteItem",
