...
const Atm = async () => {
    ...
    const { userId, userPin }: UserInput = await Inquirer.prompt([
        {
            name: "userId",
            type: 'input',
    ...
    if (!userId || !userPin) return console.log(chalk.red("Invalid input ..!"))

    while (status) {
        const { options }: { options: "Cash withdrawl" | "Balance" | "Exit" } = await Inquirer.prompt([
            {
                name: "options",
